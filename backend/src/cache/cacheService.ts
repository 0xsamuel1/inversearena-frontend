...
export interface CacheService {
  ...
  setIfGenerationIsNewer<T>(key: string, value: T, generation: number, ttlSeconds: number): Promise<boolean>;
}
...
export function createCache(client: Redis = redis): CacheService {
  return {
    ...
    async setIfGenerationIsNewer<T>(key: string, value: T, generation: number, ttlSeconds: number): Promise<boolean> {
      const generationKey = `${requireString(key, "cache.set_if_generation", "cache")}:generation`;
      const current = Number(await client.get(generationKey) ?? -1);
      if (generation < current) return false;
      await client.multi()
        .set(key, serialize(value), "EX", validatePipelineTtl(ttlSeconds, "cache.set_if_generation", "cache"))
        .set(generationKey, String(generation), "EX", validatePipelineTtl(ttlSeconds, "cache.set_if_generation", "cache"))
        .exec();
      return true;
    },
